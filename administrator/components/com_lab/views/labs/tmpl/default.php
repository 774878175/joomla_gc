	...
	<div id="filter-bar" class="btn-toobar">
		<div class="filter-search btn-group pull-left">
			<label for="filter_search" class="element-invisible">
				<?php echo JText::_('COM_LAB_SEARCH_IN_TITLE') ?>
			</label>
			<input type="text" name="filter_search" id="filter_search" placeholder="<?php 
			echo JText::_('COM_LAB_SEARCH_IN_TITLE') ?>" 
			value="<?php echo $this->escape($this->state->get('filter_search')) ?>"
			title="<?php echo JText::_('COM_LAB_SEARCH_IN_TITLE') ?>" />
		</div>
		...
		<table class="table table-striped" id="labList">
			...
			<tbody>
				<?php foreach($this->items as $i=>$item): 
					...
					$item->checked_out == 0;
					$canChange = $user->authorise('core.edit.state', 'com_lab') && $canCheckin;
					?>
				<tr class="row<?php echo $i%2;?>">
					...
					<td class="nowrap has-context">
						<?php echo JHtml::_('jgrid.published',$item->state,$i,'labs.',$canChange,'cb',
